...
void TrackletsTask::monitorData(o2::framework::ProcessingContext& ctx) // This function, monitorData, is responsible for processing TRD tracklet data and filling histograms for monitoring purposes in the ALICE O² framework
{
  ...
  for (const auto& trklt : tracklets) {
    ...
    if (!mTrackletQSupermodule[supermodule][0]) {
      mTrackletQSupermodule[supermodule][0] = std::make_unique<TH1F>(Form("TrackletQ0_SM%d", supermodule), Form("TrackletQ0_SM%d", supermodule), 100, 0, 1000);
      getObjectsManager()->startPublishing(mTrackletQSupermodule[supermodule][0].get());

      mTrackletQSupermodule[supermodule][1] = std::make_unique<TH1F>(Form("TrackletQ1_SM%d", supermodule), Form("TrackletQ1_SM%d", supermodule), 100, 0, 1000);
      getObjectsManager()->startPublishing(mTrackletQSupermodule[supermodule][1].get());

      mTrackletQSupermodule[supermodule][2] = std::make_unique<TH1F>(Form("TrackletQ2_SM%d", supermodule), Form("TrackletQ2_SM%d", supermodule), 100, 0, 1000);
